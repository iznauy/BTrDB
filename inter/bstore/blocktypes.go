...
func (v *Vectorblock) Serialize(dst []byte) []byte {
	...
	for i := 1; i < int(v.Len); i++ {
		...
		// 计算 delta 与上三条（不足 3 条有多少算多少）delta 的均值
		ddt := dt - adt
		dde := de - ade
		ddm := dm - adm
		...
		ddm <<= 2
		if dde != 0 { // 因为在频率比较固定的时序数据中，ddt 和 ddm 大概率是 0，依据此来进行数据压缩
			ddm |= 2
		}
		if ddt != 0 {
			ddm |= 1
...
func (c *Coreblock) Serialize(dst []byte) []byte {
	...
	*/
	...
	for i := 0; i < k; i++ {
		...
		if c.Addr[i] == 0 { // 如果没有这一块，那就写入一个AbsZero
...
func decompose(val float64) (e uint16, m uint64) { // 序列化成指数部分和底数部分
